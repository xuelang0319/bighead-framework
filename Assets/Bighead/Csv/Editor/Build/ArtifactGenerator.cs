...
namespace Bighead.Csv
{
    ...
    public sealed class ArtifactGenerator : IArtifactGenerator
    {
            ...
            BuildTransaction tx)
        {
            ...
            {
                SplitKey(kv.Key, out var excel, out var sheet);
                ...
                // BEFORE：扫描器产出的原始 CSV 片段
                Debug.Log("[Csv] BEFORE: " + kv.Value.Replace("\r", "").Replace("\n", "\\n").Substring(0, Mathf.Min(200, kv.Value.Length)));
                ...
                Debug.Log($"[Bighead.Csv] CSV tmp written: {csvTmp}");
                tx.Track(codeTmp);
            }
        }
